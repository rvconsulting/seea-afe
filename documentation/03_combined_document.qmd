...
implementing_countries <- country_summary |>
  filter(`Implementing the SEEA` == "Yes" | n_compiled > 0) |>
...
nca_indicators <- tribble(
  ...
  ~forest_cover_change_pct, # net forest cover change over the account period (%)
  ~forest_period_years,     # length of the account window used for annualization
  ~land_degradation_pct,    # net ecosystem degradation index or equivalent (%)
  "COD", "DRC",          NA,   94,  -6.6, 20, NA,
  "ETH", "Ethiopia",     NA,   90,    NA, NA, NA,
  "KEN", "Kenya",      15.9,   70,  -8.7, 16, NA,
  "MDG", "Madagascar",   NA,   NA,    NA, NA, 35.6,
  "MUS", "Mauritius",    NA,   NA,    NA, NA, NA,
  "RWA", "Rwanda",     23.4,   NA, -60.5, 25, NA,
  "UGA", "Uganda",     27.0,   78, -60.5, 25, NA,
  "ZAF", "South Africa", NA, 46.5,   0.8,  4, NA,
  "ZMB", "Zambia",       NA,   78,  -4.5,  5, 1.44
)
```
...
**Implementation of NCA varies across the region.** Of the 26 AFE countries assessed, 11 are implementing the SEEA. Four are at Stage III (i.e. they have sustained institutional programs for the continuous production of NCAs), four at Stage II (i.e. have produced at least one SEEA ecosystem or thematic account, in addition to at least one SEEA Central Framework account), and three at Stage I (i.e. have produced at least one SEEA Central Framework account). Three additional countries are compiling accounts outside of the formal institutional SEEA framework, and thus have no stage in this review. However, their efforts, are substantive and deserve attention. Overall, South Africa and Uganda lead with six and five compiled accounts respectively. Physical water accounts are the most common entry point, compiled by six countries.

**Across the reviewed countries, the accounts reveal a structurally coherent picture of natural capital stress.** For example, Woody biomass supplies 78 to 94 percent of national energy in the DRC, Uganda, and Zambia, which is a share that is driving forest loss across all three countries. Uganda's forest cover fell 60 percent between 1990 and 2015, while Zambia lost 1.9 million hectares of tree cover in just five years (2018–2023). Water productivity in agriculture is consistently the lowest of any sector. For instance, Rwanda's agriculture achieves 118 Rwf per cubic metre against 6,236 Rwf/m³ for mining and Uganda's agriculture earns UGX 1,458/m³ against UGX 364,299/m³ for services, which translate into those sectors being 52 and 250 times more water productive than agriculture, respectively. Mauritius has lost two-thirds of treated potable water to distribution leaks before it reaches households.

**This report pairs NCA data with other macroeconomic indicators to place these findings in economic context.** Average annual forest-cover loss is faster where agriculture is a larger share of GDP, which is consistent with forest-to-cropland conversion during structural transformation, but it does not always improve food security per capita. Uganda and Rwanda still have the highest agricultural shares and the fastest annual rates; their accounts also record a cumulative loss of about 60 percent of forest cover between 1990 and 2015. Biomass energy dependency correlates inversely with GDP per capita, making clean cooking investment a precondition for energy transition rather than a development co-benefit. Countries with higher rural population shares show faster rates of natural land cover loss, reflecting smallholder frontier expansion.
...
Source: @unceea_global_assessment_2025[].
...
country_summary |>
  ...
  scale_x_continuous(breaks = scales::breaks_width(1), expand = expansion(mult = c(0, 0.05))) +
  labs(
...
The slow growth of formally registered implementation across the region reflects primarily resource and budget constraints, including the absence of dedicated funding lines within national statistical budgets and the limited technical staffing available for a task that requires sustained multi-agency coordination. It does not reflect a lack of country interest, because in the Global Assessment, 77 percent of non-implementing countries indicated plans to begin implementation [@unceea_global_assessment_2025]. The count of accounts identified in this review also diverges from UN-reported figures in that Kenya and South Africa each have more accounts documented here than in official UN records, reflecting work not yet captured in the UN's institutional classification cycle.

## Account Coverage

@fig-heatmap provides an aggregate view of account group coverage across 14 AFE countries: the 11 formally tracked by the UN’s Global Assessment [@unceea_global_assessment_2025], plus Rwanda, the DRC, and Madagascar, whose accounts were compiled through World Bank and donor programs and are not yet registered in the UN classification. Countries are ordered left to right by total number of compiled accounts and accounts within each group are ordered by frequency of compilation. Across all implementing countries, the most widely compiled accounts are the Physical Supply and Use Tables (PSUTs) for water, followed by PSUTs for Energy and Ecosystem Extent Accounts, reflecting their relative accessibility as entry-level accounts and their relevance to the region's resource management priorities. Physical asset accounts for land follow. Beyond this initial tier, account compilation remains constrained by data availability and institutional capacity, due to the fact that monetary accounts require price data, sector-level consumption surveys, and inter-agency coordination. Ecosystem accounts appear in only 3 countries, and thematic accounts for forests and protected areas are at the same level.

```{r}
#| label: fig-heatmap
#| echo: false
#| fig-cap: "Account implementation across 14 AFE countries: the 11 tracked by the UN Global Assessment plus Rwanda, DRC, and Madagascar, whose accounts were compiled outside its scope"
...
@fig-water-gdp shows Water Productivity and Sectoral Economic Performance. That is, how many more times economic value per m³ each non-agriculture sector generates compared to agriculture, for three AFE countries with compiled water accounts. Agriculture is equivalent to the reference line (or 1x) in each respective country. Rwanda and Uganda values are derived from monetary water supply and use tables (local currency per m³, indexed) and Mauritius values are derived from World Development Indicators--WDI--[@worldbank_wdi_2026] sector Gross Value Added divided by sector water use volumes in the Physical Water Account 2021–2022 (Statistics Mauritius, 5th edition, December 2024). For context, agriculture GDP share is shown in parentheses from WDI alongside Rwanda monetary water accounts data [@nisr_rwanda_water_2019]; Uganda water accounts data [@ubos_uganda_water_2025]; and Mauritius Physical Water Account [@statsmauritius_water_2024].
...
ggplot(sector_water, aes(
  ...
  annotate("text", x = 1, y = 0.3, label = "Agriculture = 1x",
           hjust = -0.1, vjust = -0.4, size = 2.8, colour = "grey40") +
  scale_x_continuous(
    expand = expansion(mult = c(0, 0.18)),
    labels = function(x) paste0(x, "\u00d7"),
    name   = "Water productivity relative to agriculture in the same country"
  ) +
...
Water _Supply and Use Tables_ from Rwanda, Uganda, and Mauritius provide a direct measure of how efficiently different economic sectors convert water into economic value. The gaps are significant. In Uganda, the services sector generates 250 times more economic value per cubic metre than agriculture, while in Mauritius, manufacturing and services generate 214 times more. Similarly, in Rwanda, services generate 89 times more and mining 53 times more than agriculture.

The pattern holds across countries at very different income levels and with different accounting methodologies. All three countries converge on the fact that agriculture accounts for the overwhelming majority of water demand while generating the lowest return per cubic metre of any sector. In Mauritius, agriculture represents only 4 percent of GDP, but it absorbs 302 of the 344 million cubic metres abstracted from the environment annually. This finding is valuable for water pricing policy, irrigation efficiency investment, and crop diversification programs.
...
ggplot(forest_nca, aes(
  ...
  geom_point(size = 5, alpha = 0.85, shape = 21, colour = "black") +
  geom_text_repel(size = 3, colour = "grey20", box.padding = 0.4) +
  scale_x_continuous(limits = c(-3, NA)) +
  scale_y_continuous(
    limits = c(-3.2, 0.6),
    breaks = seq(-3, 0.5, by = 0.5)
  ) +
  scale_fill_gradient(
  ...
  labs(
    ...
    y = "Average annual net forest cover change\n(percentage points per year)",
  ) +
  theme_minimal(base_size = 10) +
...
Regarding energy dependence, @fig-land-rural shows the biomass share of national energy supply derived from compiled NCA energy accounts, compared against CO₂ emissions per capita, excluding LULUCF (WDI AR5 methodology, latest year) for six AFE countries with energy account data. Point color reflects GDP per capita in current USD from WDI [@worldbank_wdi_2026]. A lighter orange represents a lower value and a darker red a higher value. Countries in the upper-left combine very high biomass dependence with very low conventional CO₂ statistics, which is an apparent paradox that natural capital accounts resolve by documenting accelerating forest depletion from fuelwood harvesting for energy purposes [@worldbank_drc_forest_2025; @ubos_uganda_forest_2020; @zambia_nca_compendium_2026; @statssa_energy_2025].
...
#| message: false
#| warning: false
#| fig-cap: "Biomass share of energy supply vs CO\u2082 per capita"
...
ggplot(biomass_co2, aes(
  ...
  geom_point(size = 5, alpha = 0.8, shape = 21, colour = "black") +
  geom_text_repel(size = 3, colour = "grey20", box.padding = 0.4) +
  annotate(
    "text", x = 0.8, y = 85,
    label = "High biomass dependence,\nwith low CO\u2082 per capita\nparadox",
    size = 2.9, colour = "#9e2a00", hjust = 0, fontface = "italic"
  ) +
  scale_x_continuous(
    name   = "CO\u2082 per capita excl. LULUCF (t CO\u2082e, AR5, latest year)",
    limits = c(NA, 7.3)
  ) +
  scale_fill_gradient(
    name   = "GDP per capita\n(current USD)",
  ...
  labs(
    y = "Biomass percentage share of national energy supply",
  ) +
  theme_minimal(base_size = 11) +
  theme(legend.position = "right")
```
